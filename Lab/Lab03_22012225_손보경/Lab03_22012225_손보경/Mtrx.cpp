...
Mtrx::Mtrx(istream& fin)
{
	...
	fin >> size_row >> size_col; //파일로부터 행,열 크기 받고
	n_row = size_row;
	n_col = size_col;

	/* 입력받은 크기에 맞춰 동적 배열 생성 */
	dM = new DBLPTR[n_row];
	for (i = 0; i < n_row; i++)
	{
		dM[i] = new double[n_col];
...
Mtrx Mtrx::add(const Mtrx& mA) //call by refenence
{
	int i, j;

	Mtrx mR(n_row, n_col);   //지역변수
	mR.setName("R");
	for (i = 0; i < n_row; i++) {
		for (j = 0; j < n_col; j++) {
...
Mtrx Mtrx::multiply(const Mtrx& mA)
{
	...
	mR.setName("R");
	for (i = 0; i < n_row; i++) {  //5
		for (j = 0; j < mA.n_col; j++) {  //5
			mR.dM[i][j] = 0.0;
			for (k = 0; k < n_col; k++) {  //7
				mR.dM[i][j] += dM[i][k] * mA.dM[k][j];  //5*5 = 5*7  *  7*5
			}
		}
	}
	return mR;
