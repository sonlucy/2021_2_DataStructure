...
public:
	...
	bool isFull() { return (this->end >= this->heapPriQ_capa); /*return size() == this->capacity;*/ }
...
HeapPrioQueue<K, V>::HeapPrioQueue(int capa, string nm)
	:CompleteBinaryTree<K, V>(capa, nm), heapPriQ_capa(capa)
{ }
...
void HeapPrioQueue<K, V>::fprint(ostream& fout)
{
	if (size() <= 0)  //비어있음
	{
		...
		CompleteBinaryTree<K, V>::fprintCBT(cout);
}
