...
void Trie<E>::insert(string keyWord, E value)
{
	...
	else if ((pTN->getKey() > *keyPtr) && (*keyPtr != '\0'))
	{
		...
		pTN = pTN_New; //pTN을 다시 옮겨가서 나머지부분들을 달수있게.
		keyPtr++;
		while (*keyPtr != '\0')
		{
			//위와 동일한 방식으로 while문
...
template<typename E>
TrieNode<E>* Trie<E>::_find(string keyWord, SearchMode sm ) //지정된 keyword의 위치의 TrieNode를 반환하는 함수
{
...
template<typename E>
void Trie<E>::_traverse(TrieNode<E>* pTN, List_pVoca& list_keywords)//TreNode들을 순차적으로 방문해서 해당되는 keywords들을 List_pVoca에 담아줌
{                                                                    //pTN이 “ac”를 가리키는 경우, act, action을 list_keywords에 담아 반환하게됨.
...
template<typename E>
void Trie<E>::findPrefixMatch(string keyStr, List_pVoca& predictWords)  //접두어 일치하는 단어들 모아서 list에 담아주기
{
...
void Trie<E>::deleteKeyWord(string keyWord)
{
	...
	while (1)
	{
		if (pTN == NULL)
			break;
		if (pTN->getPrev() && pTN->getNext())  //삭제해야할 trie노드의 prex, next 둘다 존재
		{
			tmp = pTN;
		...
		else if (pTN->getPrev() && !(pTN->getNext()))  //삭제해야할 trie노드의 prev만 존재 (next없음)
		{
			tmp = pTN;
		...
		else if (!(pTN->getPrev()) && pTN->getNext()) //ptN의 prev가 NULL이고 next는 존재
		{
			tmp = pTN;
...
void Trie<E>::eraseTrie()  //마지막 key부터 역순으로 삭제
{
	...
	while (pTN != NULL)
	{
		...
		else   ///삭제해야하는 문자에 오,왼에 어떠한 TRIE노드가 없는 경우. 부모로 올라가면서 계속 삭제해야함
		{
			...
			else  //pTN->getParent() == NULL인 경우. 계속 부모로 올라가줘야함
			{
				pTN = pTN -> getPrev();
			}
			free(pTN_to_be_deleted);
