...
public:
	...
	T* removeHeapMin(); //제일 우선순위가 높은거 찾아내서 삭제하기
	T* getHeapMin();//우선순위가 높은거(root) 읽기
...
T* HeapPrioQueue<T>::getHeapMin()
{
	...
	pMinElem = (T_Entry<T>*) new T_Entry<T>;  //T_Entry를 담아낼수잇는 동적메모리생성, pMinElem가 T_Entry항목 담을 수 잇는 새로운 메모리블록 가리키게됨
