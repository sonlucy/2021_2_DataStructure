...
Quadr_Prism::Quadr_Prism(int px, int py, int w, int l, int h, /*int pri_h,*/ double ang,
	COLORREF clr, /*COLORREF ln_clr, COLORREF br_clr, int pen_thick,*/ string n)
...
void Quadr_Prism::draw(ConsolePixelFrame cp_frame)
{
	...
	start_ang_rad = angle * 3.141592 / 180.0;  //라디안으로
	start_px = center_px + height * cos(start_ang_rad);
	start_py = center_py - height * sin(start_ang_rad);
...
ostream& operator<<(ostream& fout, Quadr_Prism& pr)
{
	fout << pr.name << ": pos( " << pr.get_pos_x() << ", " << pr.get_pos_y() << ") ";
	//fout << ", line_color("; fprintRGB(fout, line_color);
	//fout << "), brush_color("; fprintRGB(fout, brush_color);
	fout << ", color("; fprintRGB(fout, pr.color);
	fout << "), width(" << pr.width << "), length(" << pr.length << "), height:("<<pr.height<<")";
...
double Quadr_Prism::getArea()
{
	double a;
	a = width * length * 2 + width * height * 2 + length * height * 2;

	return a;
}

void Quadr_Prism::draw(ostream& fout)
{
	fout << name << ": pos( " << get_pos_x() << ", " << get_pos_y() << ") ";
	//fout << ", line_color("; fprintRGB(fout, line_color);
	//fout << "), brush_color("; fprintRGB(fout, brush_color);
	fout << ", color("; fprintRGB(fout, color);
	fout << "), width(" << width << "), length(" << length << "), height:(" << height;
	fout << "), hexa_area(" << getArea() << "), hexa_volume(" << getVolume() << ")\n";
}
