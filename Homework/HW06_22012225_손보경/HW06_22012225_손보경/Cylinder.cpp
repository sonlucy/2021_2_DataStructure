...
Cylinder::Cylinder(int px, int py, int r, int ht, int ang,
...
ostream& operator<<(ostream& fout, const Cylinder& cyl)
{
	...
	fout << "), color("; fprintRGB(fout, cyl.color);
	fout << ")";
...
{
	double a;
	a = PI * radius * radius * 2 + 2 * PI * radius * height;

	return a;
}

double Cylinder::getVoume()
{
	double v;
...
void Cylinder::draw(ostream& fout)
{
	...
	fout << ", radius(" << radius << ")";
	fout << ", height (" << height;
	fout << "), Cylinder_area(" << getArea() << "), Cylinder_volume(" << getVoume() << ")\n";
