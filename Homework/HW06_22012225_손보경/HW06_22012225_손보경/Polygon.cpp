...
PolyGon::PolyGon(int px, int py, int r, int n_poly, double ang, 
...
ostream& operator<<(ostream& fout, PolyGon& pg)
{
	fout << pg.name << ": pos( " << pg.get_pos_x() << ", " << pg.get_pos_y() << ") ";
	//fout << ", line_color("; fprintRGB(fout, line_color);
	//fout << "), brush_color("; fprintRGB(fout, brush_color);
	fout << ", color("; fprintRGB(fout, pg.color);
	fout << "), radius( " << pg.radius << "), num_poly(" << pg.num_poly << ") ";
...
void PolyGon::draw(ostream& fout) 
{
