...
Star::Star(int px, int py, int r, int n_vertices, double ang, 
...
ostream& operator<<(ostream& fout, Star& st)
{
	fout << st.name << ": pos( " << st.get_pos_x() << ", " << st.get_pos_y() << ") ";
	//fout << ", line_color("; fprintRGB(fout, line_color);
	//fout << "), brush_color("; fprintRGB(fout, brush_color);
	fout << ", color("; fprintRGB(fout, st.color);
	fout << "), radius( " << st.radius << "), verticex( " << st.num_vertices << ")";

	return fout;

}

void Star::draw(ostream& fout)
{
	fout << name << ": pos( " << get_pos_x() << ", " << get_pos_y() << ") ";
	//fout << ", line_color("; fprintRGB(fout, line_color);
	//fout << "), brush_color("; fprintRGB(fout, brush_color);
